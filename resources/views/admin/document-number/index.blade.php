...
@section('title')
{{ trans('documentnumber.crud_name') }} - ARB2B
@endsection
...
<ol class="breadcrumb">
  ...
  <li><a href="#">{{ trans('documentnumber.crud_name') }}</a></li>
</ol>
<h4>{{ trans('documentnumber.crud_name') }}      
...
<div class="table-responsive">
    <table id="documentnumber-datatable" class="table table-borderless table-hover">
        <thead>
            <tr class="active text-center">
                <th>ID</th><th>{{ trans('documentnumber.organization_id') }}</th><th>{{ trans('documentnumber.current_number') }}</th><th>{{ trans('documentnumber.next_number') }}</th><th>Acciones</th>
            </tr>
        </thead>
        <tbody>
        @foreach($documentnumber as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td>{{ $item->organization->name }}</td><td>{{ $item->current_number }}</td><td>{{ $item->next_number }}</td>
                <td>
...
<script type="text/javascript">
  $(function() {
    $("#documentnumber-datatable").DataTable({
      columnDefs: [
...
<script type="text/javascript">
  function create() {
    $.ajax({
      url: "{{ url('/admin/document-number/create') }}",
      method: "GET",
  ...
  function view(id) {
    $.ajax({
      url: "{{ url('/admin/document-number/') }}/"+id,
      method: "GET",
      success: function(data) {
  ...
  function edit(id) {
    $.ajax({
      url: "{{ url('/admin/document-number/') }}/"+id+"/edit",
      method: "GET",
      success: function(data) {
