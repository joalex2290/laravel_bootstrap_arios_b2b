    ...
    <table id="office-datatable" class="table table-borderless table-hover">
        <thead>
            <tr class="active text-center">
                <th>ID</th><th>{{ trans('office.code') }}</th><th>{{ trans('office.name') }}</th><th>{{ trans('office.type') }}</th><th>Acciones</th>
            </tr>
        </thead>
        <tbody>
        @foreach($office as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td>{{ $item->code }}</td><td><a href="javascript:view({{$item->id}});">{{ $item->name }}</a></td><td>{{ $office_types[$item->type] }}</td>
                <td>
