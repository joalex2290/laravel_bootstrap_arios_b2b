...
@section('title')
{{ trans('orderdetail.crud_name') }} - ARB2B
@endsection
...
<ol class="breadcrumb">
  ...
  <li><a href="#">{{ trans('orderdetail.crud_name') }}</a></li>
</ol>
<h4>{{ trans('orderdetail.crud_name') }}      
...
<div class="table-responsive">
    <table id="orderdetail-datatable" class="table table-borderless table-hover">
        <thead>
            <tr class="active text-center">
                <th>ID</th><th>{{ trans('orderdetail.line') }}</th><th>{{ trans('orderdetail.order_id') }}</th><th>{{ trans('orderdetail.product_id') }}</th><th>Acciones</th>
            </tr>
        </thead>
        <tbody>
        @foreach($orderdetail as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td>{{ $item->line }}</td><td>{{ $item->order_id }}</td><td>{{ $item->product_id }}</td>
                <td>
...
<script type="text/javascript">
  function create() {
    $.ajax({
      url: "{{ url('/admin/order-detail/create') }}",
      method: "GET",
  ...
  function edit(id) {
    $.ajax({
      url: "{{ url('/admin/order-detail/') }}/"+id+"/edit",
      method: "GET",
      success: function(data) {
