...
<ol class="breadcrumb">
  ...
  <li><a href="#">{{ trans('city.crud_name') }}</a></li>
</ol>
<h4>{{ trans('city.crud_name') }}      
...
<div class="table-responsive">
    <table id="city-datatable" class="table table-borderless table-hover">
        <thead>
            <tr class="active text-center">
                <th>ID</th><th>{{ trans('city.code') }}</th><th>{{ trans('city.name') }}</th><th>{{ trans('city.department_id') }}</th><th>Acciones</th>
            </tr>
        </thead>
        <tbody>
        @foreach($city as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td>{{ $item->code }}</td><td><a href="javascript:view({{$item->id}});">{{ $item->name }}</a></td><td>{{ $item->department->name }}</td>
                <td>
