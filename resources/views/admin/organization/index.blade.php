...
<h4>{{ trans('organization.crud_name') }}      
...
<div class="table-responsive">
    <table id="organization-datatable" class="table table-borderless table-hover">
        <thead>
            <tr class="active text-center">
                <th>ID</th><th>{{ trans('organization.tax_id') }}</th><th>{{ trans('organization.name') }}</th><th>{{ trans('organization.comercial_name') }}</th><th>Acciones</th>
            </tr>
        </thead>
        <tbody>
        @foreach($organization as $item)
            <tr>
                <td>{{ $item->id }}</td>
                <td>{{ $item->tax_id }}</td><td><a href="javascript:view({{$item->id}});">{{ $item->name }}</a></td><td>{{ $item->comercial_name }}</td>
                <td>
...
<script type="text/javascript">
  function create() {
    $.ajax({
      url: "{{ url('/admin/organization/create') }}",
      method: "GET",
  ...
  function edit(id) {
    $.ajax({
      url: "{{ url('/admin/organization/') }}/"+id+"/edit",
      method: "GET",
      success: function(data) {
