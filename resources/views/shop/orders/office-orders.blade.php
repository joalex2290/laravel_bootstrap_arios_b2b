...
<table id="orders-datatable" class="table table-hover table-condensed">
    ...
    <tbody>
        ...
        @foreach($office->orders()->get() as $order)
        <tr>
            <td class="hidden-xs text-right">{{$order->doc_num}}</td>
            <td class="text-right">{{$order->user->name}}</td>
            <td class="text-right">{{$order->office->name}}</td>
            <td class="text-right">{{$order->catalog->name}}</td>
            <td class="text-right">{{$order_status[$order->status]}}</td>
            <td class="hidden-xs text-right">{{$order->created_at}}</td>
            <td class="hidden-xs text-right">{{$order->dispatched_at}}</td>
            <td class="hidden-xs text-right">{{$order->delivered_at}}</td>
...
<form id="order-details" action="{{route('order.details')}}" method="GET">
    ...
    <input type="hidden" name="url" value="shop/office-orders">
